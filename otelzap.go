...
func (l *Logger) logFields(
	...
	fields = append(fields, zap.Int("SeverityNumber", levelToSeverity(lvl)))
	return fields
}
...
func (s *SugaredLogger) logKVs(
	...
	for i := 0; i < len(kvs)-1; i += 2 {
		if key, ok := kvs[i].(string); ok {
			attrs = append(attrs, Attribute(key, kvs[i+1]))
		}
	}
	...
	kvs = append(kvs, zap.Int("SeverityNumber", levelToSeverity(lvl)))
	return kvs
}
